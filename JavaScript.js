...
document.addEventListener("DOMContentLoaded", () => {
    ...
    document.getElementById("searchFilterButton").addEventListener("click", () => {
        ...
        loadNewOrganization(selectedValue, jsonAllOrganizations);
    });
...
function loadNewOrganization(siteId, siteArray) {
    let selectedSite;

    // Finding the object of the selected site, to get all of his informations :
    for (let i = 0; i < siteArray.length; i += 1) {
        if (siteArray[i].Id == siteId)
            selectedSite = siteArray[i];
    }
